...
object WidgetRepo {
    ...
    suspend fun calculateDateTitle(showTomorrow: Boolean): String {
        ...
        return withContext(Dispatchers.Default) {
            ...
            val todayWeekIndex = now.dayOfWeek.formatChina()
            if (now < termStartDate) {
                //开学之前，那么计算剩余时间
                val remainDays = betweenDays(now, termStartDate)
                return@withContext "距离开学还有${remainDays}天 $todayWeekIndex"
            }
            val date = if (showTomorrow) now.plus(1, DateTimeUnit.DAY) else now
            val days = betweenDays(termStartDate, date)
            val weekIndex = date.dayOfWeek.formatChina()
            val week = (days / 7) + 1
            if (week > 20) {
    ...
    private suspend fun loadTodayCourseList(
        ...
        resultList.map {
            it.updateTime()
            val startTime = showDate.atTime(it.startTime).asInstant()
            WidgetTodayItem(
                it.courseName,
                listOf(
    ...
    private suspend fun loadTodayThingList(list: List<TodayThingView>): List<WidgetTodayItem> =
        withContext(Dispatchers.Default) {
            val showInstant = Clock.System.now()
            //过滤出今日或者明日的课程
            val showList = list
            ...
            showList.map {
                ...
                val remainDays = startDateTime.date.toEpochDays() - LocalDate.now().toEpochDays()
                val showTime = ArrayList<String>()
                if (it.saveAsCountDown) {
                    showTime.add("还剩")
                    showTime.add("${remainDays}天")
                } else {
                    showTime.add(startDateTime.time.format(Formatter.TIME_NO_SECONDS))
                    showTime.add(endDateTime.time.format(Formatter.TIME_NO_SECONDS))
                }
                WidgetTodayItem(
                    it.title,
                    showTime,
